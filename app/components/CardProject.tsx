...
export default function CardProject({ title, description, image, technologies, githubUrl, featured, slug, dict, lang }: CardProjectProps) {
    return (
        <article id="card-project" className="w-full mb-10">
            <div className={`w-full md:w-full mx-auto ${featured ? 'lg:col-span-2 lg:row-span-2' : ''}`}>
                <div className={`${featured ? 'min-h-129' : 'min-h-87.5'} bg-white border-3 border-main shadow-neo-xl relative`}>
                    {featured ? (
                    ...
                    <div className="p-4">
                        ...
                        <div className="flex gap-6 justify-end">
                            <Link href={`/${lang}/projects/${slug}`} className={`flex items-center gap-2 ${featured ? 'bg-accent-blue text-white text-xs border-3 border-main px-2 py-2 font-bold shadow-neo hover:shadow-neo-hover hover:translate-x-[2px] hover:translate-y-[2px] transition-all' : 'text-base text-sm'}`}>
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"></path></svg> {dict.moreDetails}
                            </Link>
                            {/* {githubUrl ? (
